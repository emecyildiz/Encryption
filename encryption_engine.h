...
class encryption_engine {
    public:
    bool encrypt_xor(std::filesystem::path file_path, std::string key);
    bool dencrypt_xor(std::filesystem::path file_path, std::string key);
    ...
    bool process_file(std::filesystem::path file_path, std::string key, ActionType action, CipherType cipher, bool delete_original = false);
